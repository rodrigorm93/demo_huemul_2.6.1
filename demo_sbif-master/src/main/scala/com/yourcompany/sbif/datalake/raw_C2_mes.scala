...
class raw_C2_mes(huemulBigDataGov: huemul_BigDataGovernance, Control: huemul_Control) extends huemul_DataLake(huemulBigDataGov, Control) with Serializable  {
   ...
   //configura el nombre del archivo (se pueden usar comodines)
     .setFileName ("c2{{YYYY}}{{MM}}{{Cod_Banco}}.txt")
   ...
   //Forma detallada
     .addColumn("planCuenta_id", "planCuenta_id", StringType, "Código contable. Es un campo de 7 digitos que identifica el concepto contable que se describe en el archivo Modelo-MC2.txt.")
     .addColumn("c2_Monto", "monto", DecimalType(20,2), "Total (Valor en millones de pesos chilenos)")
